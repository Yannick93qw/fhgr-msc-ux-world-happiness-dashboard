        ...
        dcc.Tab(label='Tab Two', id="tab_2_graphs", children=[html.Div([
            ...
            dbc.Row([dbc.Col(html.Div([dbc.Label("Number of bins:", html_for="heatmap_nbins"),
                     ...
                     dbc.Col(html.Div([dbc.Label("Color:", html_for="heatmap_color"),
                                       dcc.Dropdown(options=["Viridis", "Magma", "Hot", "GnBu", "Greys"], value='Viridis', id='heatmap_color', multi=False)]),width={"size": 3,"offset": 1},),
                     dbc.Col(html.Div([dbc.Label("Separated for Cluster:", html_for="heatmap_separated"),
                                       dcc.RadioItems(options=["Yes","No"], value='No', id='heatmap_separated')]),width={"size": 3,"offset": 1},)]),
                     dbc.Row([dbc.Col([dcc.Graph(id="heatmap")], width=12)])], style={"margin": "10px 25px 25px 25px"})]),])], style={"margin": "10px 25px 25px 25px"})])
...
def update_heatmap(nbins, color, separated, selected_data):
    cluster_dff = update_selected_data(selected_data=selected_data)
